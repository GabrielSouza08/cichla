...
QuestionService.prototype.Update = async(req, res, _questionRepository) => {
    ...
    if (data.status == false && data.count == 0) {
        ...
        res.json(_shared.NotificationTemplate(true, [], `Questão atualizada com sucesso!`));
    } else res.json(_shared.NotificationTemplate(false, [], `Já existe uma questão com essa descrição!`));
};

QuestionService.prototype.Get = async(res, _questionRepository) => {
    let data = await _questionRepository.Get();
    res.json(_shared.NotificationTemplate(true, data, "Lista de questões cadastradas!"));
};
...
QuestionService.prototype.GetQuantity = async(res, _questionRepository) => {
    ...
    res.json(_shared.NotificationTemplate(true, data, "Lista de questões e quantidades a partir do critério!"));
};
...
QuestionService.prototype.Activate = async(req, res, _questionRepository) => {
    ...
    res.json(_shared.NotificationTemplate(true, [], "Questão ativada com sucesso!"));
};
...
QuestionService.prototype.Disable = async(req, res, _questionRepository) => {
    ...
    res.json(_shared.NotificationTemplate(true, [], "Questão desabilitada com sucesso!"));
};
...
var resultHandlerInclude = async function(req, res, data, _questionRepository) {
    ...
    //área localizada, distinto e inativo. Então atualiza e ativa
        ...
        res.json(
            _shared.NotificationTemplate(true, [], "Questão localizada, atualizada e ativada")
        );
    }
};
