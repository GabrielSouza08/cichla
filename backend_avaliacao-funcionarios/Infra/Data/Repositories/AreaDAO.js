...
AreaDAO.prototype.GetIdByEmail = async function(description) {
    let conn = new dbConn(true);

    query = `SELECT ID_CARGO AS id
    FROM TB_CARGOS
    WHERE DS_CARGO = "${description}";`;

    return conn.query(query).then((result) => { return result[0].id });
};

AreaDAO.prototype.GetPermissions = async(id) => {
    let conn = new dbConn(true);
    let query = `SELECT 
    ID_PERMISSAO  		AS id		
...
AreaDAO.prototype.Update = async function(req, isChangePassword) {
    let user = req.body;
...
AreaDAO.prototype.ExistenceValidationByEmail = async function(email) {
    let conn = new dbConn(true);
